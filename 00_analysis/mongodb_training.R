...
# 4.5 Disconnection from database ----
...
user_base_tbl %>% toJSON(POSIXt = "mongo") %>% prettify() #date to numeric value

# Adding nested structure to mongodb
mongo_connection$insert(data = user_base_tbl)
...
# read from the other user base. good to have two, one for testing one for production
...
mongo_update_and_write_user_base <- function(user_name, column_name, assign_input,
    ...
    update_string <- user_base_tbl %>%
        ...
        # precautionary, don't want to overwrite this
        select(-user, -password, -permissions) %>%
        toJSON(POSIXt = "mongo") %>%
        str_remove_all(pattern = "^\\[|\\]$")

    # Update
    # find the user and update the values (this is our write function)
    mongo_connection$update(
        query  = query_string,
