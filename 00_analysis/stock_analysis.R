...
stock_list_tbl <- tq_index("SP500") %>%
    ...
    arrange(symbol) %>%
    transmute(label = str_c(symbol, company, sep = ", "))
...
"APPL, Apple Inc." %>% get_symbol_from_user_input()



# 3.0 GET STOCK DATA ----

from <- as.Date("2019-08-20") - days(180)
to   <- start

"AAPL" %>% tq_get(get = "stock.prices",
                  from = from,
                  to   = to) %>%
...
get_stock_data <- function(stock_symbol, from = today() - days(180),  to = today(),
...
stock_data_tbl <- get_stock_data("AAPL", mavg_short = 5, mavg_long = 8)
...
g <- stock_data_tbl %>%
    ...
    ggplot(aes(x = date, y = value, col = legend)) +
...
dump(
    ...
    file = "00_scripts/stock_analysis_function.R",
    append = FALSE)
